...
public class AddAppointmentController implements Initializable {
    ...
    public void appSaveButtonClick(ActionEvent actionEvent) {
        ...
        try {
            LocalDateTime tempStart = appStartDate.getValue().atTime(

                    Integer.parseInt(appStartTimeHours.getSelectionModel().getSelectedItem().toString().substring(0, 2)),
                    Integer.parseInt(appStartTimeMinutes.getSelectionModel().getSelectedItem().toString().substring(0, 2))
            );

            LocalDateTime tempEnd = appEndDate.getValue().atTime(
                    Integer.parseInt(appEndTimeHours.getSelectionModel().getSelectedItem().toString().substring(0, 2)),
                    Integer.parseInt(appEndTimeMinutes.getSelectionModel().getSelectedItem().toString().substring(0, 2))
            );
            ...
            int tempContactID = appContactBox.getSelectionModel().getSelectedItem() == null ? -1 :
                    ((Contacts) appContactBox.getSelectionModel().getSelectedItem()).getContact_ID();
            ...
            try {
                ...
                if (duration.toHours() > 14) {
                    ...
                    alert.setContentText("Meeting must be between 8:00 AM and 10:00 PM EST.");
                    alert.showAndWait();
                    return;
                }
                if (startEST.getHour() < 8 || startEST.getHour() >= 22 ||
                        endEST.getHour() < 8 || endEST.getHour() >= 22) {
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setTitle("Error");
                    alert.setHeaderText("Invalid Time");
                    alert.setContentText("The meeting time must be between 8:00 AM and 10:00 PM EST.");
                    alert.showAndWait();
                    return;
                }

                ObservableList<Appointments> custAppointments = AppSQL.getAppointmentsContacts(tempContactID);
                for (Appointments app : custAppointments) {
                    ...
                    if (tempContactID == app.getContact_ID() && appDate.equals(tempStart.toLocalDate())) {
                        if (tempStart.toLocalTime().isAfter(appStartTime.minusMinutes(1)) && tempStart.toLocalTime().isBefore(appEndTime.plusMinutes(1))) {
                            Alert alert = new Alert(Alert.AlertType.ERROR);
                            alert.setTitle("Error");
                            alert.setHeaderText("Invalid");
                            alert.setContentText("Contact is already scheduled for another meeting during this time.");
                            alert.showAndWait();
                            return;
                        }
                        if (tempEnd.toLocalTime().isAfter(appStartTime.minusMinutes(1)) && tempEnd.toLocalTime().isBefore(appEndTime.plusMinutes(1))) {
                            Alert alert = new Alert(Alert.AlertType.ERROR);
                            alert.setTitle("Error");
                            alert.setHeaderText("Invalid");
                            alert.setContentText("Contact is already scheduled for another meeting during this time.");
                ...
                if (tempTitle != "" && tempDescription != "" && tempLocation != "" && tempType != "" && tempStart != null && tempEnd != null) {
                    AppSQL.addAppointment(tempID, tempTitle, tempDescription, tempLocation, tempContactID, tempType, tempStart, tempEnd, tempCustomerID, tempUserID);
